...
module.exports = (router, app) => {
  ...
  const lru = new AsyncLRU({ max: 10, load })
  const lruDocs = new AsyncLRU({ max: 10, load })
  ...
  router.post('/api', koaBody, async (ctx) => {
    ...
    // TODO: separate doc cache from the rest
    // and only invalidate the rest
    lru.clear()
  })
